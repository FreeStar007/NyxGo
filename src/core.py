#!/usr/lib/nyxgo_venv/bin/python3
import os
import httpx
import json
...
# 输入文件检测
def checkout_nyxbot(_, current) -> bool:
    ...
    if not os.path.exists(current) or os.path.isdir(current):
        raise ValidationError("", reason="路径无效啊而且也不能是文件夹")
        
    starter_command.append(current)
    return True
...
def install_napcat() -> bool:
    ...
    info(f"NapCat搞定，输入“xvfb-run -a qq --no-sandbox -q <你的QQ号>”来启动，会让你扫码登录，随后在它给的WebUI地址中配置一个WS服务器，消息格式选Array，然后自己输入一个端口，记住这个地址，例如6666端口地址就是ws://127.0.0.1:6666，然后在NyxBot配置界面的{Choices.STARTING_MODE.value}选择{Choices.CLIENT_MODE.value}再设置{Choices.CONNECTION_URL.value}去连接NapCat的WebSocket服务器地址就行了")
...
def checkout_env() -> bool:
    ...
    if os.name == "posix":
        ...
        if not shell("command -v qq > /dev/null", complex_mode=True):
...
def main() -> None:
    ...
    if ask(Confirm("to_shell", message="需要进入临时Shell环境以启动QQ机器人框架吗？", default=True)):
        warn("10秒后进入临时Shell环境，让你启动一下QQ机器人框架并去配置，结束后输入CTRL加A然后按D来回到前台")
        sleep(10)
